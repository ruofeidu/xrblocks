...
async function requestWebcam() {
  try {
    ...
    if (err && err.name === 'NotAllowedError') {
      console.info(
        '[magic_window] webcam permission denied; staying on simulator camera.'
      );
    } else {
      console.warn(
        '[magic_window] webcam unavailable; staying on simulator camera.',
        err
      );
    }
    return false;
...
export async function installWebcamFallback(xb) {
  ...
  console.log('[magic_window] webcam fallback installed.');
}
